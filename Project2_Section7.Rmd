...
## Confusion Matrix
Data is unbalanced. Using a threshold of 0.5, the FNR was 0.551, so we lowered the threshold.
```{r}
##predicted probs for test data
preds <- predict(reduced, newdata=test, type="response")

##confusion matrix with threshold of 0.2
table(test$goodQuality, preds>0.2)
```
```{r}
error_rate <- (899+169)/nrow(test)
accuracy <- (8575+1155)/nrow(test)
FPR <- 899/(8575+899)
FNR <- 169/(169+1155)
TPR <- 1155/(1155+169)
TNR <- 8575/(8575+899)
precision <- 1155/(1155+899)
```
